...
## エラー 6 個目（2026-04）: 設定テーマまわり（表示ずれ・Duplicate callback・ラグ）

### 6-1. 「選択中」は DB 通りなのに全体の色が先頭テーマのまま

【原因】

- `theme-preview-name` / `theme-preview-store` は `load_theme()` で更新されても、**`#bootswatch-theme` の `href`（Bootswatch CSS）** を同じタイミングで更新していなかった
- カードクリック時のコールバックだけが `href` を書き換えていたため、未クリックではレイアウト初期のスタイルシートのまま残り得た

【対策】

- `/settings` では **`theme-preview-store` を正**にし、`theme-store` と `pathname` を見て **`bootswatch-theme` の `href` を clientside で決める**（`assets/themeScroll.js` の `applyBootswatchFromStores`）
- `theme-preview-store` は **ルート `app.py` のレイアウトに常設**し、設定ページ専用レイアウトからは重複定義しない（clientside の Input が常に解決できるようにする）

### 6-2. テーマカードの「選択中」ハイライトが先頭カードに付く

【原因】

- `mark_active_card` が `State({"type": "theme-card", "theme": ALL}, "id")` の **列挙順**で `className` のリストを組み立てていた
- Dash の `ALL` では **返却リストの順はレイアウト上のコンポーネント順**に対応するため、列挙順とずれると **`active` が別カードに対応付く**（先頭に見える）

【対策】

- `BOOTSWATCH_SWATCH.keys()` と同じ順で id をソートしてから `className` を組み立てる（`_sorted_theme_card_ids` 等）
- `/settings` 同期コールバックで **カードの `className` をまとめて返す**場合も同じ順序を使う

### 6-3. ブラウザコンソール: `Duplicate callback outputs`（`bootswatch-theme.href`）

【症状】

- Dash renderer が `Duplicate callback outputs` を出し、`bootswatch-theme.href` が「既に使用中」と表示される

【原因】

- **Dash 4** では、`Input("_pages_location", "pathname")` を共有する **複数のサーバーコールバック**が、同じ **`bootswatch-theme.href`** を出力すると、**`allow_duplicate=True` を付けていても登録時に衝突する**（`_sync_nav_store_and_theme` と `sync_settings_theme_preview_from_pathname` の両方から `href` を返していたのが典型）

【対策】

- **同一 `pathname` トリガーで同じ `href` を複数のサーバーコールバックから返さない**
- 設定ページ用の `href` 同期は **clientside に寄せる**か、**1 本のサーバーコールバックに統合**する（`callback_context` で分岐）

### 6-4. 体感ラグ

【メモ】

- `href` 更新を clientside に寄せたことで、**サーバー往復後にブラウザでリンクが差し替わる**ため、ごく短いラグを感じることがある
- 挙動が正本（DB / プレビュー）と一致していれば許容範囲として扱える。気になる場合は Network で `bootstrap.min.css` の取得タイミングを確認する
