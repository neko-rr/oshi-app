...
- **フォントサイズ（スマホ）**
  - 主要入力欄は **16px 以上**を推奨（iOS Safari のフォーカス時の自動ズーム抑制と可読性の両立）。バーコード手入力（`input-custom`）は実装で **16px** を指定済み
- **ブラウザのオートフィル**
  - 背景が上書きされて文字が薄くなるテーマがあるため、**`-webkit-autofill` 向けの `box-shadow` / `caret-color` / `-webkit-text-fill-color`** を共通 CSS に寄せる（ネイティブ `input.form-control` と **`.dash-input-container` 内の `input.dash-input-element`** の両方）
- **縦方向（上下欠け）とフレックス**
  - テキストボックスの **行高・上下 padding・最小高さ** を CSS で下限保証する（日本語ベアライン・テーマ差での欠け対策）
  - **アイコンと入力を横並びにする行**では、親を **`align-items-stretch`** とし、**入力を縮めない**。アイコン列だけ **`align-self-center`** とする（例: [`features/receipt_location_tag/components.py`](features/receipt_location_tag/components.py)）

#### 3.2) Dash `dcc.Input` / `dcc.Textarea` と実装正本（`assets/styles.css`）

Dash では `dcc.Input` / `dcc.Textarea` の `className`（例: `form-control`, `input-custom`）は **実 `<input>` / `<textarea>` ではなく、ラッパー `div`** に付与される。実要素は **`input.dash-input-element` / `textarea.dash-input-element`** である。

| DOM の目安 | 役割 |
|---|---|
| `div.dash-input-container.dash-input` +（任意）`form-control` / `input-custom` / `me-2` 等 | ラッパー。枠線・角丸・背景の多くはここに載せる |
| `input.dash-input-element` または `textarea.dash-input-element` | **入力値の表示・行高・内側 padding** はここを正とする |

**典型トラブル（2026-04 対応済み）**: Bootstrap の **`.form-control` がラッパー `div` に付いたとき、`padding` がラッパーに効き、内側の `.dash-input-element` のレイアウト高さが数 px まで潰れる**ことがある。

**実装正本**（[`assets/styles.css`](assets/styles.css)・`.page-container` スコープ）の要点:

- **`.page-container .dash-input-container.dash-input`** … `display: flex`（縦積み）、`align-items: stretch`、`min-height: 2.75rem`、**`padding: 0`**（ラッパーへの Bootstrap `padding` を打ち消し）
- **`.page-container .dash-input-container.dash-input input.dash-input-element`** … `min-height: 2.75rem`、`height: auto`、`line-height: 1.5`、`padding: 0.5rem 0.75rem`、`font-size: 1rem`、背景は透明（枠・面はラッパー側）、`color: var(--bs-body-color)`
- **textarea 用** … 上記に相当する **`textarea.dash-input-element`** ルール（`resize: vertical` 等）
- **`input-custom`（バーコード手入力）** … ラッパー `.input-custom` は **`padding: 0`** + flex、**内側**に `font-size: 16px` と縦余白。フォーカスリングは **`:focus-within`**（内側 input にフォーカスがあるときラッパーに表現）
- **ネイティブ** … `dbc.Input` 等で **`input` 本体に `form-control` が付く**場合は **`.page-container input.form-control`** ルール（`form-control-color` は除外）
- **`select.form-control`** … 同上スコープで縦メトリクスを揃える（`multiple` は除外）
- **`register-review-static-mount`** 内のフォームも、ルートレイアウト上は **`.page-container` 配下**のため **同一ルールが適用**される

**dbc 実装メモ**

- `dbc.Input` は多くの場合 **`input` 本体に `form-control`** が付く。親の文字色継承に注意し、色付き面では **`text-body` の切れ目**や `input-custom` 相当の色指定で同化を避ける
...
- **`dcc.Input` / `dcc.Textarea`** に `className="form-control"` を付けたときは、**スタイルの当て先がラッパーか `.dash-input-element` か**を混同しない。新規の見た目調整は原則 [`assets/styles.css`](assets/styles.css) の **`.page-container .dash-input-container…`** に寄せる
...
- **Dash テキスト入力**で DevTools 上、**`input.dash-input-element` の高さが極端に小さい（例: 10px 未満）**になっていないか。なっていればラッパーに **`form-control` 由来の `padding`** が残っていないか、`assets/styles.css` の **`.dash-input-container.dash-input { padding:0 }`** が効いているかを確認する
...
| `focus-ring-color`, `focus-ring-width` | `.input-custom:focus-within`、ボタン focus-visible | ラッパーに `input-custom` が付く Dash では **`:focus-within`**。固定青の直接指定を避ける |
| `input-bg`, `input-text`, `input-placeholder` | `.input-custom` ラッパー（`background` / `color`）、内側は `body-color` | 入力値の可読性。ラッパーは flex + `padding:0`、**内側 `.dash-input-element` に縦余白** |
| （縦欠け・ネイティブ） | `.page-container input/textarea/select.form-control` | `dbc` 等で **要素本体**に `form-control` が付く場合。`line-height` / `padding-block` / `min-height`（`form-control-color` 除外） |
| （縦欠け・Dash dcc） | `.page-container .dash-input-container.dash-input`、内側の `.dash-input-element`（`input` / `textarea`） | ラッパー **`padding:0`**、内側に **`min-height: 2.75rem`** 等。§3.2 参照 |
