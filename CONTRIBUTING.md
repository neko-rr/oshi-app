...
10. Auth 変更前: skill `official-docs-first`。**DB 変更前: skill `db-schema-change`**。デプロイ/本番 env: **`deploy-change`**。製品フロー変更前: **`product-spec-sync`**。shared↔API パス: **`api-contract-sync`**。UI 変更前: **`design-change`**（本決定前比較は **`design-lab`**、本番採用は **`design-adoption`**、要望は **`design-feedback`**、コントラスト等は **`design-a11y`**）。モバイル UI: **`design-mobile`**。変更後: `secure-change-checklist`（該当時）
...
エージェントはチャット内で常にブラウザスクショできるとは限らない。UI の機械スクショは **PR の Playwright ジョブ**（Artifacts: `ui-screenshots`）を正とする。
