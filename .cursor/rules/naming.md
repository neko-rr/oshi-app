# 命名規則（Next.js + FastAPI monorepo）

Apply Mode: Always Apply

# 目的

新規・変更コードの **ファイル名・識別子・API パス・用語** を一系統に揃える。  
なりゆきで混ざった旧 Dash 名は **新規採用しない**。DB の既存列名は安易に改名しない。

**構成の正**: [docs/migration/v2/rules/file_structure.md](../../docs/migration/v2/rules/file_structure.md)  
**用語対応（旧→新）**: [docs/migration/v2/glossary.md](../../docs/migration/v2/glossary.md)  
**API 形**: [docs/migration/v2/rules/api_contract.md](../../docs/migration/v2/rules/api_contract.md)
...
| DB テーブル・列（既存） | **既存の `snake_case` を維持** | `registration_product_information` |
...
- 例: `{ "members_id": "...", "registration_product_id": 1 }`
...
| フェーズ | 方針 |
|----------|------|
| 新規 monorepo コード | **本 rule 100%** |
| 旧 Dash ファイルが残る間 | 旧名のまま修正可。**大きな改名 PR は機能移植と分離** |
| DB | 用語は glossary。物理名変更はマイグレーション計画が先 |
