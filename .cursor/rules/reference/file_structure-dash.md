# ARCHIVE — Dash 期のファイル構成（新規設計に使うな）

> **このファイルは歴史参照のみ。現行アプリの正本ではない。**  
> v2 の配置の正: `docs/migration/v2/rules/file_structure.md` + `architecture.mdc` / `platform.mdc`（`apps/web` / `apps/api`）。  
> 製品要件の正: `docs/product/`。DB の正: `docs/db/`。  
> `controller.py` / `server.py` / Dash Pages / Bootswatch 前提を新規に増やすな。

# 基本品質基準（Dash 期・参考）

Apply Mode: （無効・アーカイブ）

# このファイルは Cursor への指示用でした。【v2 では Always Apply しない】
