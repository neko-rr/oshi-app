...
本計画では、プロジェクトの [.cursor/rules/spec.md](.cursor/rules/spec.md)（機能・スマホ・dbc・Bootswatch・Bootstrap Icons）および [.cursor/rules/file_structure.md](.cursor/rules/file_structure.md)（URL・構成）と **1対1で矛盾しない** ように [DESIGN.md](DESIGN.md) を埋める。
...
- [DESIGN.md](DESIGN.md) に **Colors / Typography / Spacing / Components / Elevation / Guidelines** がすべて実質内容を持ち、spec の技術スタックと矛盾しない。
