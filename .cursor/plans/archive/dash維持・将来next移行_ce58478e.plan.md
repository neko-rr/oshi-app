...
- **認証・DB**: Supabase Auth + Postgres **RLS** + 行の **`members_id`**（[`services/supabase_client.py`](services/supabase_client.py) のユーザークライアントで `auth.uid()` が効く）を継続する。
- **当面**: UI は Dash（[`app.py`](app.py)、`features/*/controller.py`、`pages/`）。
...
1. **現在ユーザーIDの取得を1か所に集約**  
   現状は [`services/photo_service.py`](services/photo_service.py)、[`services/tag_service.py`](services/tag_service.py)、[`services/registration_service.py`](services/registration_service.py) などに `_current_members_id()` が分散。  
...
2. **モジュール先頭の `supabase` 固定を避ける**  
   [`app.py`](app.py) 23行付近の `supabase = get_supabase_client()` は、import 時点では Flask リクエストコンテキストが無く、**ユーザークライアントにならない**可能性がある。  
...
3. **`theme_service` のように `members_id` を引数で受ける層**  
   [`services/theme_service.py`](services/theme_service.py) は呼び出し側依存が強い。**内部では必ず `get_current_members_id()` をデフォルトにする**か、**Dash 側は常に同じヘルパー経由でだけ呼ぶ**ルールを決める（呼び出し漏れ対策）。

4. **RLS とアプリ条件のギャップ棚卸し**  
   共有マスタ想定（例: [`services/icon_service.py`](services/icon_service.py) が触る `icon_tag`）とユーザーデータを **一覧表で分類**（意図的に緩い / 緩いのは暫定で直す）。README の `owner_id` 表記と実装の `members_id` のズレも、将来のオンボーディングコストなので **どちらかに用語統一**（ドキュメントかスキーマのどちらか）。
...
3. **デモ用フォールバックの方針を明文化**  
   [`services/data_service.py`](services/data_service.py) のインメモリ `PHOTOS_STORAGE` は開発便利だが、本番挙動と二重管理になりやすい。**「Supabase 無し時だけ」** など条件を README かコメントで固定し、将来 FastAPI では別設定フラグに寄せる。
...
3. **環境変数と秘密情報の境界**  
   引き続き **サービスロールはサーバー専用**（[`get_secret_client`](services/supabase_client.py) は診断・管理用途に限定）。Next 化後も **ブラウザに渡さない** 前提を維持。
...
5. **Cookie ドメイン / CORS / リフレッシュ**を本番 URL に合わせて整理（いまの [`server.py`](server.py) HttpOnly 方針を FastAPI 側に移すか、Next BFF に寄せるかを決める）。
