...
- アプリ全体は [`app.py`](app.py) で `prevent_initial_callbacks="initial_duplicate"` かつ `suppress_callback_exceptions=True`。
...
- **`prevent_initial_call="initial_duplicate"`** → 重複 Output まわりの「初回どれを許すか」が絡む（[`features/review/controller.py`](features/review/controller.py) の `process_tags` やギャラリーの `gallery-products-store` など）。
...
1. ブラウザ DevTools の Network で、対象ルートごとに **`/_dash-update-component` の件数・順序・ペイロード** を記録する（[`Cursor.md`](Cursor.md) の必須シナリオ: `/` と `/gallery` に加え、負荷が大きいなら **`/register/review`**）。
2. サーバー側では `DASH_DEBUG=1` のときだけ詳細ログ（[`services/debug_log.py`](services/debug_log.py)）を使い、コールバック名や分岐を対応付ける。

**補足（コールバック外の初回コスト）**: [`pages/home.py`](pages/home.py) の `render_home()` は **レイアウト生成時に同期実行**され、`get_product_stats` / `get_random_product_with_photo` が走る。これは `_dash-update-component` に出ないが **TTFB に直結**するため、コールバック調査と分けて記録する。
...
**運用**: 上記を **全 PR のレビュー観点**として使う。リポジトリ側の転記先は [`Cursor.md`](Cursor.md) の「PRレビュー観点（Step 1b: セキュリティゲート）」セクション（要約チェックリスト）。

### 認可とデータ境界（サーバー側は最後の砦）

- **統合コールバック**は「1往復で楽」になる一方、分岐が肥大化すると **認可チェックの抜け**が起きやすい。DB/Storage へ触れる経路は、統合後も **現在ユーザー（RLS 有効クライアント）前提**を崩さない（[`services/supabase_client.py`](services/supabase_client.py) の利用方針と整合）。
...
- **`registration-store`**: 画像 base64 等が `_dash-update-component` のリクエスト/レスポンスに載り得る。統合で **同じデータがより多くの Output に触れる**だけなら新規漏えいではないが、**ログ・エラートレース・デバッグ print** にストア全体を出さない（[`services/debug_log.py`](services/debug_log.py)・`DASH_DEBUG`・`AUTH_DEBUG` は本番オフを検証手順に明記）。
...
- **`theme-store` → `bootswatch-theme` href**（[`components/theme_utils.py`](components/theme_utils.py)）: `localStorage` 改ざんで **任意 URL の CSS を読み込む**リスクに備え、クライアント側でも **許可リスト（既存 `BOOTSWATCH_THEMES`）**外は無視する、またはサーバー保存値を正とする。CDN は信頼境界として文書化。
...
|------|------|-------------------------|----------|------------------|
| [`app.py`](app.py) `_redirect_register` | はい（常に `PreventUpdate`） | 未指定（False） | 無実処理だが **POST は発生** | **クライアントリダイレクト**または `prevent_initial_call=True` + 初回のみ別経路などで **往復削減** |
| [`app.py`](app.py) `_reset_store_on_register` | はい | `False` | `nav-history-store` と条件付きで `registration-store` リセット | [`app.py`](app.py) の pathname 系 **1本化**（下記 C）で POST 集約 |
| [`app.py`](app.py) `sync_theme_on_navigation` | はい | `False` | `load_theme()` → Supabase の可能性 | pathname 系と **統合**するか、テーマは **初回のみクライアント＋遅延サーバー同期**を検討 |

### B. テーマ（[`components/theme_utils.py`](components/theme_utils.py)）
...
### C. ギャラリー [`pages/gallery/index.py`](pages/gallery/index.py)
...
### D. ダッシュボード [`pages/dashboard.py`](pages/dashboard.py)
...
### E. ギャラリー詳細 [`pages/gallery/detail.py`](pages/gallery/detail.py)

| 箇所 | 初回 | 備考 |
|------|------|------|
| `_on_query_change`（`Input("_pages_location", "search")`） | デフォルト False | `search` が `None` なら `PreventUpdate`。**空文字 `""` のときは** [`render_detail_page`](pages/gallery/detail.py) がエラーUIを返し **無駄な往復**になり得る |

**改善案**: コールバック先頭で `pid` が無い場合は `PreventUpdate`、または `prevent_initial_call` と `pathname` の組み合わせで **クエリ確定後だけ**実行。

### F. レビュー [`features/review/controller.py`](features/review/controller.py)（`/register/review` 表示時に Output が揃う）
...
- [`features/photo/controller.py`](features/photo/controller.py) / [`features/barcode/controller.py`](features/barcode/controller.py): 主要アクションは `initial_duplicate` または `PreventUpdate` 前提。
- [`pages/register/select.py`](pages/register/select.py): `prevent_initial_call=True`
- [`features/color_tag/controller.py`](features/color_tag/controller.py): `prevent_initial_call=True`
...
2. **中**: [`app.py`](app.py) の pathname 系3本を **1コールバック・複数 Output** に統合（テーマ同期とストア更新の責務を明確に分離したまま同一往復に載せる）。  
