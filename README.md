...
- **ギャラリー**（`/gallery` / `/gallery/detail`）: 登録済み製品のサムネ一覧・詳細。一覧ではテキスト検索とカラータグのスウォッチ（OR フィルター）。詳細では **カラータグ（最大7・junction `registration_product_color_tag`）・カテゴリータグ・収納場所の付与**（マスタの名称・アイコン等の CRUD は設定画面）。保存後は一覧用 `dcc.Store`（session）を再取得。Dash の `gallery-products-store` / `gallery-color-filter` は [app.py](app.py) ルートに常設し、詳細ページのコールバックとも整合させている。
- 設定（テーマ切替・登録件数・全データ削除）。
- **タグ設定（ユーザー別・RLS）**（`/settings` から各画面へ）
  ...
  - **登録フローとの連携**: カラータグは登録ストアから利用可能。登録フローの確認画面で `category_tag_id` / 収納場所を選ぶ UI は未接続（設定でマスタ整備、ギャラリー詳細で付与可能）。
...
- 現実の収納場所の管理（拡張）
  - 設定でユーザー別の収納場所タグは編集可能。ギャラリーでは一覧チップ表示・詳細での付与まで対応。所在の地図連携などさらなる組み込みは今後
...
- **主要エンティティ**: `photo`（サムネ・高解像の object path）、`registration_product_information`（製品情報・バーコード・価格・`category_tag_id` / `receipt_location_id` など）、`registration_product_color_tag`（製品とカラータグの多対多・slot 1..7）、`color_tag` / `category_tag` / `receipt_location`（ユーザー別タグ・収納場所）、`icon_tag`（Bootstrap Icons クラス名のマスタ。カテゴリ／収納場所のピッカー用）、`theme_settings`（テーマ永続化）。いずれもユーザー境界は `members_id`（= `auth.uid()`）と RLS を前提。
  - **タグ系の現状**: カラー・カテゴリ・収納場所は設定画面から CRUD 可能（[services/tag_service.py](services/tag_service.py)、[services/icon_service.py](services/icon_service.py)）。ギャラリー詳細からの FK・色付与の更新は [services/product_assignment_service.py](services/product_assignment_service.py)（`category_tag_id` / `receipt_location_id` を検証のうえ REST PATCH）と [services/product_color_tag_service.py](services/product_color_tag_service.py)（junction の delete + insert）。スキーマの正本は [.cursor/rules/database_configuration.md](.cursor/rules/database_configuration.md)。カテゴリ／収納場所は `display_order`、プリセット用 `slot`（1..6）、ユーザーがプリセット行を消した slot の再作成抑止用の補助テーブル（`*_preset_slot_dismissed`）などを [supabase/migrations/](supabase/migrations/) で管理。
  - **DB マイグレーション（運用メモ）**: `registration_product_information` は更新日時列が `updated_date` のみである一方、旧トリガーが `NEW.updated_at` を参照すると REST の PATCH が失敗する。修正は [supabase/migrations/20260419160000_registration_product_information_updated_date_trigger.sql](supabase/migrations/20260419160000_registration_product_information_updated_date_trigger.sql) を適用すること。
...
| 変数名                                    | 説明                                                                                   |
| ----------------------------------------- | -------------------------------------------------------------------------------------- |
| `PUBLIC_SUPABASE_URL`                     | Supabase の **Project URL**（例: `https://<project-ref>.supabase.co`）。末尾に `/rest/v1` を付けないのが望ましい（誤記載時は [services/supabase_client.py](services/supabase_client.py) でプロジェクトルートへ正規化）。 |
...
- ギャラリーでタグ保存や製品情報の更新が失敗する場合: Supabase に未適用のマイグレーションがないか確認（特に `registration_product_information` の `updated_date` 用 BEFORE UPDATE トリガー。エラー例: PostgREST 400 / `record "new" has no field "updated_at"`）。適用後はサーバー再起動は不要だが、ブラウザの再読み込みで Store を揃えるとよい。
- ギャラリーの色フィルターが意図せずオンになる場合は、リポジトリ最新の [pages/gallery/index.py](pages/gallery/index.py)（スウォッチ `n_clicks` の誤発火防止・フィルタ Store の正規化・一覧再取得時のフィルタ解除）を反映しているか確認。
...
│   ├── gallery/           # ギャラリー一覧・詳細（タグ検索・色フィルター・詳細でのタグ付与）
│   └── settings/          # 設定（index, color_tags, receipt_location_tags, category_tags 等）
├── components/            # レイアウト・ナビ・横断 UI
├── features/              # 機能別コールバック（barcode, photo, review, color_tag, receipt_location_tag, category_tag, gallery 等）
...
│   ├── product_assignment_service.py  # 製品への category / receipt FK 更新（REST PATCH）
│   ├── product_color_tag_service.py   # 製品へのカラータグ slot 付与（junction）
│   └── photo_service.py   # Storage アップロード・signed URL・DB CRUD・ギャラリー用 SELECT
